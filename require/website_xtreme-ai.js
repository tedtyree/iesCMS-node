...
class webEngine {
    ...
    async CustomTags(ret,cms) {  
        try {
            ...
            switch (ret.Tag.toLowerCase()) {
                case "tag1":
                    content.append("tag1_content");
                    break;
                case "tag2":
                    content.append("tag2_content");
                    break;
                case "tag123":
                    content.append("this is tag123_content");
                    break;
                case "notion-databases":
                    let records = (await this.fetchDatabases())?.data || [];
                    let html = records.map(record => {
                        let icon = (record?.icon?.file?.url) ? '<img src="' + record.icon.file.url + '" alt="icon" style="height:20px;width:auto;vertical-align:middle;margin-right:5px;">' : '';
                        return `<li>` + icon + ` ${record.title} [${record.id}] Notion_id: ${record.notion_id}</li>`;
                      }).join('\n');
                    content.append(`<ul>\n` + html + `\n</ul>`);
                    content.append('<p>' + JSON.stringify(records) + '</p>'); // debug debug
                    break;
                default:
                    ret.Processed=false;
                    break;
    ...
    CreateHtml(cms) { //async
        ...
        if (this.invalidSiteID(cms)) { reject('ERROR: Incorrect SiteID. [ERR5175]'); return; }
    ...
    async fetchDatabases() {
        const url = 'http://localhost:53001/api/databases';
  
        try {
            const response = await fetch(url);
    
            if (!response.ok) {
                let errmsg1 = `HTTP error! status: ${response.status} [ERR5544]`;
                console.log(errmsg1);
                cms.logError(errmsg1);
                return {};
            }
            ...
            errmsg2 = `Fetch error: ${error.message} [ERR5545]`;
            console.log(errmsg2);
            cms.logError(errmsg2);
            return {};
        }
  }
