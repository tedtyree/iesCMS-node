...
'*************************** NOTE!!!!!!!!!!!!
'*************************** CODE FOR UNIVERSAL ADMIN/TEMPLATES IS MARKED WITH "UNIV"  MODIFY-HERE

'*******
'******* wObject Web Engine v0.7 (default.aspx)
...
<!-- #include virtual="\admin\src\RememberMe.cfg" -->
...
Dim cpID as String, cpPWD as String, cpStatus as String=""
Dim BrandPrefix as String  '**** MODIFY-HERE  "UNIV"  
...
wInit()
DupSessionLogin(Session)
RememberMe() '*** Runs the basic remember me algorithm and gets the RememberMe token (if needed)
...
If PageID="" Then 
	...
	If Left(sHost,2)="m." AND DefaultMobileTAB<>"" Then PageID=DefaultMobileTAB
End If
...
'**** MODIFY-HERE  "UNIV"
BrandPrefix=BrandID & "_"
If BrandID="" Then BrandPrefix=""
...
'If LCase(Trim(Request.ServerVariables("HTTPS"))) = "on" Then
  '*** HTTPS requires that the user be logged in...
	...
	If UserLoggedIn() Then
	  ...
	  wLevel=CInt2(Session(wWorld & "-UserLevel"),0)
...
'******* Process a Form?
Dim FormProcessedFlag as Boolean=False, LogFormFlag as boolean=True, EmailFormFlag as boolean=True, p1err as String=""
ON ERROR RESUME NEXT
...
If formID<>"" Then
  ...
  ReadFormFields()
  cpID=trim(objFields("captchaid"))
  If cpID<>"" Then
		cpPWD=trim(objFields("captcha"))
		cpStatus=CheckCaptcha(cpID,cpPWD,wWorld)
		objFields("captchaStatus")=cpStatus
  End If
	
  '*** PROCESS form...
  CustomForm(FormID,FormProcessedFlag,LogFormFlag,EmailFormFlag, p1err)
  If p1err<>"" Then 
		strDefMsg=strDefMsg & vbNewLine & p1err & vbNewLine
		ErrMsg=p1err
  End If
  
  If FormProcessedFlag=False AND PageID="form_submit" Then
  ...
  '*** LOG/EMAIL form...
  If FormProcessedFlag=True Then

	If LogFormFlag=True Then SaveFormToLog(FormID)

	'*** Setup for email notification
	'*** Use default for FORM_Subject, FORM_SendTo
	'FORM_Fields=""   '*** FUTURE: if needed
	If EmailFormFlag=True AND (cpStatus="" OR cpStatus="OK") Then GoSendEmail()

	'*** FUTURE: Check for errors/Display results
	If cpStatus<>"OK" AND cpStatus<>"" Then
	  ErrMsg="Invalid verification code."
	  
	  PageID=Trim(objFields("OnCaptchaGoTo") & "")
	  If PageID="" Then PageID="form_submit_captcha"
	Else
	  If ErrMsg="" Then
  ...
  If PageID=LCase(LOGIN_Page) Then
	...
	If wLevel>0 Then
		...
		If InStr(LCase(sGoto),"logout=true")>0 Then sGoto=""
		If LCase(Trim(request.form("RememberMe") & ""))="true" Then SetRememberMe(uName,uPwd)
		If sGoto<>"" Then
			Session("GotoPage")=""
...
'*** Get GALLERY SPECIFIC INFO
	...
	'** For now... always do this... FUTURE: Only on certain pages; Gallery, GalleryPhoto, etc.
...
'	PageID="ShowPhoto" '*** If the Photo ID is speicified, then this puts the photo/description on the page.
'End If

'*** Process USE_WIKI_FILE and wiki_file_found to determine action...
wikiFileName=genFolder & "\" & PageID & ".cfg"

'******** CHANGES BELOW: MODIFY-HERE - UNIV
If AllowAdminContent Then
  If not(wiki_file_found(True)) Then wikiFileName=UnivContentFolder & "\" & BrandPrefix & PageID & ".cfg"
  If not(wiki_file_found(True)) Then 
	wikiFileName=UnivContentFolder & "\" & PageID & ".cfg"
	wiki_file_found(True)  '*** Have to check one last time, but end result of check is used below
  End If 
End If
...
If ((Product_Page<>"") AND (LCase(PageID)=LCase(Product_Page))) Then
	SetPEnv()
...
<script language="vb" runat="server">
	...
	cnt = 0
        Do While nRS.Read()
			'*** LIMIT THE DISPLAY TO 500 RECORDS!!! MODIFY-HERE TKT 3/10/2012 COMCAST
			If cnt>500 Then
				rsp=rsp & "<TR><TD colspan=" & ProductsPerRow & " align='center' style='background-color:#D0D0D0;'><B>MORE RECORDS</B> - Only first 500 records have been displayed.</TD></TR>"
				Exit Do
			End If
	...
	Else
	  If Trim(inTemplate)<>"" Then
		...
		'*** Check if file exists first in local Templates folder
		inTemplatePath= TemplateFolder & "\layout_" & inTemplate & ".cfg"
		If not(FILE.Exists(inTemplatePath)) Then
			'*** NOW we check the Universal Templates folder
			inTemplatePath= UnivTemplateFolder & "\layout_" & BrandPrefix & inTemplate & ".cfg"
			If not(FILE.Exists(inTemplatePath)) Then inTemplatePath= UnivTemplateFolder & "\layout_" & inTemplate & ".cfg"
			If not(FILE.Exists(inTemplatePath)) Then
				'*** FILE NOT FOUND!
				inTemplatePath=""  
				ErrMsg="Failed to locate template file: " & inTemplate & ".cfg" & " (#147)"
				cntType="error"
			End If
		End If
	...
	Dim subTag as String, p as Integer, processed as boolean
	...
	subTag=""
	subTag=GetNextTag(Tag)

	'Response.Write("DEBUG: Tag=" & Tag & "<br>") '**** DEBUG DEBUG DEBUG
	GenerateTag=""
	
	processed=CustomSwitch(GenerateTag,Tag,SubTag)
	If processed=True Then Exit Function
	ON ERROR GOTO MainSwitchError
...
'		GenerateTag=GenAdminLinks()
	  ...
	  Case "logged_in"
		...
		If Trim(Session(wWorld & "-uID") & "")<>"" AND Trim(Session(wWorld & "-UserName") & "")<>"" Then
		  GenerateTag="Logged In: " & Session(wWorld & "-UserName") & _
	  ...
	  Case "adminmsg"   '***** (MODIFY_HERE - message/link if IS admin!)
		GenerateTag=""
		If UserIsAdmin() Then GenerateTag=subTag
	  ...
	  Case "user_name"
		GenerateTag=Trim(Session(wWorld & "-UserName") & "")
	  Case "user_id"
		GenerateTag=Trim(Session(wWorld & "-uID") & "")
	  Case "user_level"
		GenerateTag=Trim(Session(wWorld & "-UserLevel") & "")
	  ...
	  Case "submenu"  '**** Menu tag looks up "menu_sub_{{PageID}}.cfg"
	  ...
	  Case "subpage"  '**** UNIV: MODIFY-HERE
		'*** FUTURE: Handle ability to read Wiki objects here, too (currently, subpage must be a file)
		Dim tWikiPage as Object, tContentText as String, tWikiFileName as String
		GenerateTag=""
		tWikiFileName=genFolder & "\" & SubTag & ".cfg"
		If not(File.Exists(tWikiFileName)) Then tWikiFileName=UnivContentFolder & "\" & BrandPrefix & SubTag & ".cfg"
		If not(File.Exists(tWikiFileName)) Then tWikiFileName=UnivContentFolder & "\" & SubTag & ".cfg"	
		If File.Exists(tWikiFileName) Then	
			ParseWikiFile(tWikiPage,tContentText,tWikiFileName,False)
			GenerateTag=Trim(tContentText)
		End If
		tWikiPage=Nothing
...
'************************** ...PHOTO GALLERY END
	  ...
	  Case ELSE   '*** Blank tag or invalid tag... return null string
	  	AdminSwitch(GenerateTag,Tag,SubTag) '*** UNIV - NOTE: TAG VALUES SET BY THIS ADMIN SWITCH CAN BE OVERRIDDEN BY THE SWITCH ABOVE!
		'*** FUTURE: Many of the tags above can be moved into AdminSwitch
	End Select
	Exit Function
...
'***************************************************************************** 
'**** UNIV MODIFY-HERE : Modified Function 
   Function WriteMenu(ByVal sMenuName as String) as String
	...
	WriteMenu=""
	sMenuPath= TemplateFolder & "\menu_" & sMenuName & ".cfg"
	If CheckMMenu=True Then
	  Dim sMenuPath2=TemplateFolder & "\menu_" & sMenuName & "_" & PageID & ".cfg"
...
'***************************************************
   ...
   End Function
Dim CaptchaID as String=""
Dim MAX_CAPTCHAS as integer = 9999
Dim CaptchaFolder as String=""
Dim CaptchaURL as String=""
Dim CaptchaPrefix as String="c"
Dim CaptchaSuffix as String=".gif"
'*** {+{+captcha}} returns the entire field, hidden field, image, etc. in a table format (ie. SubTag="")
'*** {+{+captcha:title}} returns title string only
'*** {+{+captcha:field}} returns <input...> field only
'*** {+{+captcha:hidden}} returns hidden CaptchaID field only
'*** {+{+captcha:img}} returns Captcha <img> tag only
'*** {+{+captcha:imgurl}} returns the image URL only
'*** {+{+captcha:id}} returns the selected CaptchaID only
'*** only one Captcha per page.  You can make multiple calls to Captcha() and it uses the same CaptchaID selected on the first call.
'*** Can handle up to MAX_CAPTCHAS Captchas (1 through MAX_CAPTCHAS)
Function Captcha(cSelect as String) as String
	Dim n as Double, i as Integer, c as Integer, f as String, cLast as Integer
	Dim safety as Integer=999

	Captcha=""
	If CaptchaFolder="" Then
		CaptchaFolder=baseFOLDER & "\captcha"
		CaptchaURL="/" & wWorld & "/captcha"
		If not(DIRECTORY.Exists(CaptchaFolder)) Then 
			CaptchaFolder=SERVER_FOLDER & "\captcha"
			CaptchaURL="/captcha"
		End If
	End If
	...
	If CaptchaID="" Then
		RANDOMIZE
		n=((MAX_CAPTCHAS * Rnd) + 1)
		i=CInt(n)
		cLast=MAX_CAPTCHAS+99
		c=MAX_CAPTCHAS
		Do
		  If i<cLast Then
		    '*** Check for captcha 
...
'Response.Write("Check for " & f & "<br>")
	...
	If CaptchaID="" Then
		...
		If FILE.Exists(f) Then CaptchaID="1"
	End If
	
	'*** Must have selected a Captcha in order to continue...
	If CaptchaID="" Then Exit Function
	
	'*** Generate the HTML that was requested...
	SELECT CASE LCase(Trim(cSelect))
	  CASE ""
	    Captcha="<table id='captchatable' border=0 cellpadding=0 cellspacing=0><tr><td>Please type the characters in the image:&nbsp;</td><td colspan=2><img border=0 src='" & CaptchaURL & "/" & CaptchaPrefix & CaptchaID & CaptchaSuffix & "'></td></tr>" & _
	  ...
	  CASE "id"
		Captcha=CaptchaID
	  'CASE ELSE  '*** Return blank when field is invalid
	END SELECT
...
'*****************************************************************************
  ...
  If PageID<>"" Then
	...
	If mViewLevel<OverrideMinViewLevel and PageID<>LCase(LOGIN_Page) Then mViewLevel=OverrideMinViewLevel
	allowView=False
	If mViewLevel>0 Then RememberMeLogin()  '*** Here we attempt to login via RememberMe because the page requires permissions
	If wLevel>=mViewLevel Then allowView=True
	'*** SPECIAL CODE TO ALLOW VIEWER/OWNER TO VIEW PAGE
	If LCase(Trim(wWikiPage.Param("Owner") & ""))=LCase(Trim(Session(wWorld & "-uObjID") & "")) Then allowView=True
	If LCase(Trim(wWikiPage.Param("Viewer") & ""))=LCase(Trim(Session(wWorld & "-uObjID") & "")) Then allowView=True
  ...
  ON ERROR GOTO 0 
   ...
   Sub TransferToPage(sFileName as String)
	...
	If LCase(PageID)=LCase(INVALID_PAGE) Then Response.Status="404 Not Found"
   ...
   Sub ParseWikiFile(ByRef rWikiPage as Object, ByRef rContentText as String, ByRef rWikiFileName as String, AllowTransfer as Boolean)
	...
	If mViewLevel>0 Then
	  ...
	  If wLevel>=mViewLevel Then allowView=True
	  '*** SPECIAL CODE TO ALLOW VIEWER/OWNER TO VIEW PAGE
	  If LCase(Trim(rWikiPage.Param("Owner") & ""))=LCase(Trim(Session(wWorld & "-uObjID") & "")) Then allowView=True
	  If LCase(Trim(rWikiPage.Param("Viewer") & ""))=LCase(Trim(Session(wWorld & "-uObjID") & "")) Then allowView=True
	  '************ END - ALLOW VIEWER/OWNER TO VIEW
   ...
   Sub GetLoginIf()
	...
	If Trim(Session(wWorld & "-UserName") & "")<>"" Then
...
'   End Sub
   ...
   '*** UNIV MODIFY-HERE : Modified Function wiki_file_found()
   Function wiki_file_found(optional ForceCheck as Boolean=False) as Boolean
     If (wiki_file_flag=0) OR (ForceCheck=True) Then
...
'*************************************************************************************** ReadFormFields()
	...
	Dim fldList as Object, sBold as String="", sBold2 as String="", form_w As String=""
	...
	'*** Add Footer...
	strDefMsg=strDefMsg & vbNewLine & vbNewLine
	form_w=Request.Form("form_w")
    If LCase(Trim(form_w))<>LCase(Trim(wWorld)) Then
		strDefMsg=strDefMsg & "(w:" & form_w & ":" & wWorld & ")" & vbNewLine & vbNewLine
	Else
		strDefMsg=strDefMsg & "(w:" & form_w & ")" & vbNewLine & vbNewLine
	End If
...
'*************************************************************************************** IfObjField()
	Function IfObjField(sParameter as String) as String
		IfObjField=""
		If ObjFields.ContainsKey(sParameter) Then IfObjField=ObjFields(sParameter)
	End Function
...
'*************************************************************************************** SetPEnv()
'*** Kept here in case a specific World needs to do something different with the DB connection (not moved to adminFunctions.cfg)
	Sub SetPEnv()
	 If pEnv is Nothing Then pEnv=new wObjects.wObjEnv(wWorld)
	End Sub
	
'*************************************************************************************** UserIsAdmin()
'*** UNIV MODIFY-HERE - New Function
	Function UserIsAdmin() as Boolean
	  UserIsAdmin=False
	  If wLevel>=DefaultMinAdminLevel Then UserIsAdmin=True
	End Function
	
'*************************************************************************************** CInt2()
'*** UNIV MODIFY-HERE : Removed Function CInt2() - moved to adminFunctions.cfg

</script>

<!-- UNIV MODIFY-HERE - NEW SWITCH AND FUNCTIONS IN tagFunctions.cfg FILE -->
<!-- #include virtual="\admin\src\tagFunctions.cfg" -->
<!-- #include virtual="\admin\src\adminFunctions.cfg" -->
<!-- #include virtual=".\src\CustomFunctions.cfg" -->
