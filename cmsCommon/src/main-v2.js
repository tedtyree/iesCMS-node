...
function initpage() {
	var i,img,itd,tdarray,a;

	// new idea - create custom function to override nFlag, sFlag, hFlag, hsFlag if needed.
	if(typeof window.wInitParams == 'function') {window.wInitParams();}

	a=initpage.arguments;
	if (a.length>=1) {pageid=a[0];}
	if (a.length>=2) {submenu=a[1];}
	if (submenu!="") { pageid=submenu; }
//	else { pageid=pageid.toLowerCase(); }
	...
	// FUTURE: PRELOAD ROLLOVER IMAGES - EACH DIV WITHIN MainMenu and other menus
...
function selectmnu() {
   ...
   if (sbtn) { sbtn.className = "mainmenu-s";}
}
